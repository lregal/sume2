    ...
    >
      ...
      <v-list>
        <template v-for="(item) in items">
          <v-list-item
            v-if="checkPermission(item.guard) && !item.children"
            v-model="item.model"
            :key="item.text"
          ...
          >
            ...
            <template v-if="!$store.state.sidebarMini">
              <v-list-item v-for="(child, i) in item.children" :key="i" :href="'#' + item.route + child.route"
                           class="link">
...
.main-sidebar
  ...
  ::-webkit-scrollbar-thumb
    background: rgb(93 172 74);
    border-radius: 2px;
  /* Handle on hover */
  ::-webkit-scrollbar-thumb:hover
    background: rgb(0 131 252);
</style>
